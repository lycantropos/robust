...
def coordinates_to_segments(coordinates: Strategy[Real]) -> Strategy[Segment]:
    return (to_pairs(coordinates_to_points(coordinates))
            .filter(pack(ne)))


def coordinates_to_points(coordinates: Strategy[Real]) -> Strategy[Real]:
    return to_pairs(coordinates)


segments_strategies = numbers_strategies.map(coordinates_to_segments)
...
def to_maybe_intersecting_segments(segments: Strategy[Segment]
                                                       ...
                                                       Segment]]:
    def to_scaled_segments_pair(segment_with_scale: Tuple[Segment, Real]
